@extends('layouts.app')

@section('body')
    <div class="row ">
        <div class="col-12">
            <div class="mb-6">
                <h1 class="fs-3 mb-4">Accident & Injury</h1>
                <hr>
                <div class="row g-3 mb-5">
                    <div class="col-md-4">
                        <div class="card">
                            <div class="card-header pt-2 d-flex justify-content-between align-items-center">
                                <h6 class="card-title">
                                    <i class="ti ti-plus"></i> Add New Accident & Injury
                                </h6>
                            </div>
                            <div class="card-body">
                                <form id="accidentForm" method="POST">
                                    @csrf

                                    <div class="row g-3">
                                        <div class="col-md-12">
                                            <label class="form-label mb-1 text-dark fs-14 fw-medium">Accident & Injury: <span class="text-danger">*</span></label>
                                            <input type="text" name="name" class="form-control" oninput="var words = this.value.split(' '); for(var i = 0; i < words.length; i++){ words[i] = words[i].substr(0,1).toUpperCase() + words[i].substr(1); } this.value = words.join(' ');" required>
                                        </div>
                                    </div>

                                    <!-- Form Footer Actions -->
                                    <div class="offcanvas-footer mb-1 mt-4 pt-3 border-top">
                                        <div class="d-flex justify-content-between gap-2">
                                            <button type="reset" class="btn btn-light px-3">Clear</button>
                                            <button type="submit" class="btn btn-outline-success px-3">
                                                <i class="fas fa-save me-1"></i> Save Data
                                            </button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-8">
                        <div class="card">
                            <div class="card-header pt-2 d-flex justify-content-between align-items-center">
                                <h6 class="card-title">
                                    <i class="ti ti-server"></i> List of Accident Injuries
                                </h6>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive mt-2 p-2">
                                    <table id="injurylistab" class="table table-hover" style="width: 100%">
                                        <thead class="">
                                            <tr>
                                                <th>Nature of Injury</th>
                                                <th>Status</th>
                                                <th class="text-center" width="7%">Action</th>
                                            </tr>
                                        </thead>
                                        <tbody style="font-size: 10pt;">
                                            
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Center modal content -->
    <div class="modal fade" id="injuryModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myCenterModalLabel">Edit Nature of Injury</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="injuryForm" method="POST">
                        @csrf

                        <!-- Hidden ID Field -->
                        <input type="hidden" name="id" id="injuryId">

                        <div class="row g-3">
                            <div class="col-md-12">
                                <label class="form-label mb-1 text-dark fs-14 fw-medium" for="editNatureinhury">Nature of Injury: <span class="text-danger">*</span></label>
                                <input type="text" name="name" class="form-control" id="editNatureinhury" oninput="var words = this.value.split(' '); for(var i = 0; i < words.length; i++){ words[i] = words[i].substr(0,1).toUpperCase() + words[i].substr(1); } this.value = words.join(' ');" required>
                            </div>
                        </div>

                        <!-- Form Footer Actions -->
                        <div class="offcanvas-footer mb-1 mt-4 pt-3 border-top">
                            <div class="d-flex justify-content-between gap-2">
                                <button type="button" class="btn btn-secondary px-3" data-bs-dismiss="modal">Close</button>
                                <button type="submit" class="btn btn-success px-3">
                                    <i class="fas fa-save me-1"></i> Update Changes
                                </button>
                            </div>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
@endsection
